...
import com.burbit.demo1.backend.rrhh.services.IEmpleadoService;
...
public DataTablesOutput<Empleado> findAll(@Valid @RequestBody DataTablesInput input) {
return empleadoService.findAll(input);
}
...
try
{
obj = empleadoService.findById(id);
if (obj == null)
{
...
public ResponseEntity<?> update(@Valid @RequestBody Empleado objParam, @PathVariable Long id)
{
Empleado currentObj = this.empleadoService.findById(id);
Empleado updatedObj = null;
Map<String, Object> response = new HashMap<>();
if (currentObj == null)
{
...
try
{
currentObj.setCargo(objParam.getCargo());
currentObj.setNombre(objParam.getNombre());
currentObj.setApellidoPaterno(objParam.getApellidoPaterno());
currentObj.setApellidoMaterno(objParam.getApellidoMaterno());
currentObj.setEmail(objParam.getEmail());
currentObj.setDireccion(objParam.getDireccion());
currentObj.setFechaNacimiento(objParam.getFechaNacimiento());
currentObj.setDni(objParam.getDni());
currentObj.setSexo(objParam.isSexo());
currentObj.setCondicion(objParam.getCondicion());
updatedObj = empleadoService.save(currentObj);
response.put("mensaje", "El registro ha sido actualizado con éxito!");
response.put("Empleado", updatedObj);
}
catch (DataAccessException e)
{
